	...
	<div>
		<div class="register_view">
			<el-form :model="registerForm" class="register_form">
				...
				<div class="list_item">
					<div class="list_label">维护账号：</div>
					<el-input class="list_inp"
						 v-model="registerForm.weihuzhanghao" 
						 placeholder="请输入维护账号"
						 type="text"
						/>
				</div>
				<div class="list_item">
					<div class="list_label">维护密码：</div>
					<el-input class="list_inp"
						 v-model="registerForm.weihumima" 
						 placeholder="请输入维护密码"
						 type="password"
						 />
				</div>
				<div class="list_item">
					<div class="list_label">确认维护密码：</div>
					<el-input class="list_inp" v-model="registerForm.weihumima2" type="password" placeholder="请输入确认维护密码" />
				</div>
				<div class="list_item">
					<div class="list_label">维护人姓名：</div>
					<el-input class="list_inp"
						 v-model="registerForm.weihurenxingming" 
				...
				<div class="list_item">
					...
					<el-select 
						...
						placeholder="请选择性别"
						>
						<el-option v-for="item in weihurenyuanxingbieLists" :label="item" :value="item"></el-option>
					</el-select>
				</div>
...
<script setup>
	...
	const tableName = ref('weihurenyuan')
	...
	const weihurenyuanxingbieLists = ref([])
	const init=()=>{
		weihurenyuanxingbieLists.value = "男,女".split(',')
	}
	...
	const handleRegister = () => {
		...
		if((!registerForm.value.weihuzhanghao)){
			context?.$toolUtil.message(`维护账号不能为空`,'error')
			return false
		}
		if((!registerForm.value.weihumima)){
			context?.$toolUtil.message(`维护密码不能为空`,'error')
			return false
		}
		if(registerForm.value.weihumima!=registerForm.value.weihumima2){
			context?.$toolUtil.message('两次密码输入不一致','error')
			return false
		}
		if((!registerForm.value.weihurenxingming)){
			context?.$toolUtil.message(`维护人姓名不能为空`,'error')
			return false
		}
