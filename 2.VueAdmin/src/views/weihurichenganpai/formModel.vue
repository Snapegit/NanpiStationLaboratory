				...
				<el-row>
					<el-col :span="12">
						<el-form-item label="设备名称" prop="shebeimingcheng">
							<el-select
								class="list_sel"
								:disabled="!isAdd||disabledForm.shebeimingcheng?true:false"
								v-model="form.shebeimingcheng" 
								placeholder="请选择设备名称"
								@change="shebeimingchengChange">
								<el-option v-for="(item,index) in shebeimingchengLists" :label="item" :value="item">
								</el-option>
							</el-select>
						</el-form-item>
					</el-col>
...
<script setup>
	...
	const shebeimingchengChange=()=>{
		context?.$http({
			url: `follow/shiyanshebei/shebeimingcheng?columnValue=` + form.value.shebeimingcheng,
			method: 'get'
		}).then(res=>{
			if(res.data.data.shebeibianhao){
				form.value.shebeibianhao = res.data.data.shebeibianhao
			}
			if(res.data.data.xinghao){
				form.value.xinghao = res.data.data.xinghao
			}
		})
	}
	//富文本
