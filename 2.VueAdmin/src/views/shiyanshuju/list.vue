	...
	<div>
		<div class="app-contain">
			<div class="list_search_view">
				<el-form :model="searchQuery" class="search_form" >
					<div class="search_view">
						...
						<div class="search_box">
							<el-input class="search_inp" v-model="searchQuery.shiyanmingcheng" placeholder="实验名称"
								clearable>
							</el-input>
						</div>
					</div>
					<div class="search_view">
						...
						<div class="search_box">
							<el-date-picker
								class="search_date"
								clearable
								v-model="searchQuery.shiyanriqiTimes"
								type="daterange"
								format="YYYY-MM-DD HH:mm:ss"
								value-format="YYYY-MM-DD HH:mm:ss"
								range-separator="至"
								start-placeholder="实验日期开始时间"
								end-placeholder="实验日期结束时间"
								/>
						</div>
					</div>
					<div class="search_view">
						...
						<div class="search_box">
							<el-input class="search_inp" v-model="searchQuery.yangpinmingcheng" placeholder="样品名称"
								clearable>
							</el-input>
						</div>
					</div>
				...
				<br>
				<div class="btn_view">
					<el-button type="success" @click="addClick" v-if="btnAuth('shiyanshuju','新增')">新增</el-button>
					<el-button  v-if=" btnAuth('shiyanshuju','查看')" type="info"  :disabled="selRows.length==1?false:true" @click="infoClick(null)">详情</el-button>
					<el-button type="primary" :disabled="selRows.length==1?false:true" @click="editClick" v-if=" btnAuth('shiyanshuju','修改')">修改</el-button>
					<el-button type="danger" :disabled="selRows.length?false:true" @click="delClick(null)"  v-if="btnAuth('shiyanshuju','删除')">删除</el-button>
				</div>
			</div>
			<br>
			<el-table
				...
				<el-table-column
					...
					label="科研人员姓名">
					<template #default="scope">
						{{scope.row.keyanrenyuanxingming}}
					</template>
				</el-table-column>
				<el-table-column
					 :resizable='true' 
				...
				<el-table-column
					...
					label="实验状态">
					<template #default="scope">
						{{scope.row.shiyanzhuangtai}}
					</template>
				</el-table-column>
				<el-table-column label="操作" width="300" :resizable='true' :sortable='true' align="left" header-align="left">
					<template #default="scope">
						<el-button type="info" v-if=" btnAuth('shiyanshuju','查看')" @click="infoClick(scope.row.id)">详情</el-button>
					</template>
				</el-table-column>
			</el-table>
			<el-pagination 
...
<script setup>
	...
	const getList = () => {
		...
		params['sort'] = 'id'
		params['order'] = 'desc'
		if(searchQuery.value.shiyanmingcheng&&searchQuery.value.shiyanmingcheng!=''){
			params['shiyanmingcheng'] = '%' + searchQuery.value.shiyanmingcheng + '%'
		}
		if(searchQuery.value.shiyanriqiTimes&&searchQuery.value.shiyanriqiTimes.length){
			params['shiyanriqistart'] = searchQuery.value.shiyanriqiTimes[0]
			params['shiyanriqiend'] = searchQuery.value.shiyanriqiTimes[0]
		}
		if(searchQuery.value.yangpinmingcheng&&searchQuery.value.yangpinmingcheng!=''){
			params['yangpinmingcheng'] = '%' + searchQuery.value.yangpinmingcheng + '%'
		}
		context?.$http({
			url: `${tableName}/page`,
			method: 'get',
