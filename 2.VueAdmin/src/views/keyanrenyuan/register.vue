	...
	<div>
		<div class="register_view">
			<el-form :model="registerForm" class="register_form">
				...
				<div class="list_item">
					<div class="list_label">科研账号：</div>
					<el-input class="list_inp"
						 v-model="registerForm.keyanzhanghao" 
						 placeholder="请输入科研账号"
						 type="text"
						/>
				</div>
				<div class="list_item">
					<div class="list_label">科研密码：</div>
					<el-input class="list_inp"
						 v-model="registerForm.keyanmima" 
						 placeholder="请输入科研密码"
						 type="password"
						 />
				</div>
				<div class="list_item">
					<div class="list_label">确认科研密码：</div>
					<el-input class="list_inp" v-model="registerForm.keyanmima2" type="password" placeholder="请输入确认科研密码" />
				</div>
				<div class="list_item">
					<div class="list_label">科研人员姓名：</div>
					<el-input class="list_inp"
						 v-model="registerForm.keyanrenyuanxingming" 
				...
				<div class="list_item">
					...
					<el-select 
						...
						placeholder="请选择性别"
						>
						<el-option v-for="item in keyanrenyuanxingbieLists" :label="item" :value="item"></el-option>
					</el-select>
				</div>
...
<script setup>
	...
	const tableName = ref('keyanrenyuan')
	...
	const keyanrenyuanxingbieLists = ref([])
	const init=()=>{
		keyanrenyuanxingbieLists.value = "男,女".split(',')
	}
	...
	const handleRegister = () => {
		...
		if((!registerForm.value.keyanzhanghao)){
			context?.$toolUtil.message(`科研账号不能为空`,'error')
			return false
		}
		if((!registerForm.value.keyanmima)){
			context?.$toolUtil.message(`科研密码不能为空`,'error')
			return false
		}
		if(registerForm.value.keyanmima!=registerForm.value.keyanmima2){
			context?.$toolUtil.message('两次密码输入不一致','error')
			return false
		}
		if((!registerForm.value.keyanrenyuanxingming)){
			context?.$toolUtil.message(`科研人员姓名不能为空`,'error')
			return false
		}
