...
<template>
	<div>
			...
			<div class="list_search_view">
				<el-form :model="searchQuery" class="search_form" >
					...
					<div class="search_view">
						...
						<div class="search_box">
							<el-input class="search_inp" v-model="searchQuery.shebeimingcheng" placeholder="设备名称"
								clearable>
							</el-input>
						</div>
					</div>
					<div class="search_view">
						...
						<div class="search_box">
							<el-select
								class="search_sel"
								clearable
								v-model="searchQuery.shebeizhuangtai" 
								placeholder="设备状态"
								>
								<el-option v-for="item in shebeizhuangtaiLists" :label="item" :value="item"></el-option>
							</el-select>
						</div>
					</div>
				...
				<br>
				<div class="btn_view">
					<el-button type="success" @click="addClick" v-if="btnAuth('shiyanshebei','新增')">新增</el-button>
					<el-button  v-if=" btnAuth('shiyanshebei','查看')" type="info"  :disabled="selRows.length==1?false:true" @click="infoClick(null)">详情</el-button>
					<el-button type="primary" :disabled="selRows.length==1?false:true" @click="editClick" v-if=" btnAuth('shiyanshebei','修改')">修改</el-button>
					<el-button type="danger" :disabled="selRows.length?false:true" @click="delClick(null)"  v-if="btnAuth('shiyanshebei','删除')">删除</el-button>
				</div>
			</div>
			<br>
			<el-table
				...
				<el-table-column label="操作" width="300" :resizable='true' :sortable='true' align="left" header-align="left">
					<template #default="scope">
						<el-button type="info" v-if=" btnAuth('shiyanshebei','查看')" @click="infoClick(scope.row.id)">详情</el-button>
						<el-button v-if="btnAuth('shiyanshebei','维护')" type="success" @click="shebeiweihuCrossAddOrUpdateHandler(scope.row,'cross','','','')">维护</el-button>
					</template>
				</el-table-column>
			</el-table>
			<el-pagination 
		...
		<shebeiweihuFormModel ref="shebeiweihuFormModelRef" @formModelChange="formModelChange"></shebeiweihuFormModel>
	</div>
</template>
<script setup>
	...
	const getList = () => {
		...
		if(searchQuery.value.shebeimingcheng&&searchQuery.value.shebeimingcheng!=''){
			params['shebeimingcheng'] = '%' + searchQuery.value.shebeimingcheng + '%'
		}
		if(searchQuery.value.shebeizhuangtai&&searchQuery.value.shebeizhuangtai!=''){
			params['shebeizhuangtai'] = searchQuery.value.shebeizhuangtai
		}
		context?.$http({
			url: `${tableName}/page`,
			method: 'get',
	...
	//搜索
	const shebeizhuangtaiLists = ref([])
	const getshebeizhuangtaiLists = () => {
		shebeizhuangtaiLists.value = '使用中,空闲'.split(',')
	}
	getshebeizhuangtaiLists()
	const searchClick = () => {
	...
	import shebeiweihuFormModel from '@/views/shebeiweihu/formModel'
	const shebeiweihuFormModelRef = ref(null)
    const shebeiweihuCrossAddOrUpdateHandler = (row,type,crossOptAudit,statusColumnName,tips,statusColumnValue) => {
		...
		nextTick(()=>{
			shebeiweihuFormModelRef.value.init(row.id,'cross','维护',row,'shiyanshebei',statusColumnName,tips,statusColumnValue)
		})
    }
	//初始化
	const init = () => {
